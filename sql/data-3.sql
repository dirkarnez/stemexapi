...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'C - Ninja Obby', 'Work on your particular skills while navigating this tricky obstacle course!', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Roblox Intermediate'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'D-Riddle Ruin', 'Crack the codes inside the pyramid to set yourself free!', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Roblox Intermediate';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'E-Platform Game Design', 'Avoid red obstacles and collect coins in this 2D platform Roblox game.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Roblox Intermediate'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Python Simon Says', 'Students are going to have a comprehensive understanding on the data structure in Python. They will advance their skills in using Python editor. Apply their understanding in data structure and engineering design process to create games as challenges.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Python Intermediate'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Python Flappy & Snake Games', 'Students learn how to utilize other python resources. They will concentrate on coding the movement of objects using vectors and control. A lot of logic training will be involved in understanding the conditions in gaming and how to solve them in coding.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Python Intermediate'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Python Discord Bot', 'Students will learn to creating a discord bot and how to do with language processing. In the course, students will have a chance to do some works related to the concept of neural network.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Python Advanced'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'D-Python Machine Learning', 'Students will learn how to use tensflow and the use of the module keraus. They are going to learn how to build up a machine learning module to identify different clothing from thousands of images.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Python Advanced'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Yolo', 'We will learn how to use the yolov3 model to detect objects present in an image. It will help differentiate different objects.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Python Master'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Tic tac toe', 'We will learn about graphical user interface in this project, learn about Tkinter and use it to make a game GUI of tic tac toe and we will learn logic of the game design .', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Python Master'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'D-Photo Editing App', 'We will learn about the pillow library which is used for image processing, we will learn about how we can edit images using python.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Python Master'
AND files.`physical_file_name` = '';
...
-- Coding Scratch
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Cartoon & mini-Game Creation', 'Learn by doing and problem solving. Make characters move, jump and sing.', `id`
FROM curriculum_entries, files 
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - The Pico Show / Hide and Seek', 'With Scratch, you can program your own interactive stories, games, and animations — and share your creations with others in the online community. Here you will use function and variables to create a dance party with music and background.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Coding Scratch Intermediate'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Cyber Robotics 101', 'Want to experience virtual robotics but scared it is too difficult? In this course you will learn all the basic topics of robotics to jumpstart your virtual robotic journey.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Cyber Virtual Robotics Introductory'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Physics with Ruby', 'Want to experience virtual robotics but scared it is too difficult? In this course you will learn all the basic topics of robotics to jumpstart your virtual robotic journey.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Cyber Virtual Robotics Introductory'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Tour with Ruby', 'With ruby''s excellent navigation tools, help Ruby explore through thin and dangerous road. This course will encourage students to plan for the most efficient route through using different tools such as color sensors, precision manuevers and more', `id`
FROM curriculum_entries, files 
...
-- Lego Robotics 
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Let’s Get Moving', 'Learn about the basics of mechanical engineering all based around the idea of moving, and moving very quickly. In this course, students will build various models, such as ships and race cars, in order to learn about how motors and gears function.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Lego Robotics Elementary - Wedo'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Wild Animals', 'Learn about mechanical engineering based on the animal kingdom. In this course, students will build various models, such as lions and birds, in order to learn about how motors and gears function, as well as little facts about the animals themselves.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Lego Robotics Elementary - Wedo'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'C - Rescue Heroes', 'Learn about mechanical engineering based on natural disasters. In this course, students will build various models, such as a helicopter, in order to learn about how motors and gears function, as well as how natural disasters can be prevented and how people can be rescued.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Lego Robotics Elementary - Wedo'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Missions to Mars', 'This course is designed to introduce students to the world of not only building, but also programming basic robots. Throughout this course the students will explore different ways in which a robot could be utilized to explore a distant planet.', `id`
FROM curriculum_entries, files 
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Ocean Missions', 'In the Ocean Missions curriculum, students will be introduced to the world of robotics in an interesting and engaging way. The goal is to teach students about the building and programming aspects of robotics as it relates to real-world issues in ocean exploration.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Lego Robotics Advanced - EV3'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Robot Sergeons', 'The Robot Surgeons curriculum is designed to introduce students to the world of not only building, but also programming basic robots. Throughout this course, the students will explore different ways in which a robot could be utilized in the medical field.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Lego Robotics Advanced - EV3'
AND files.`physical_file_name` = '';
...
-- VEX Robotics
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - High Rise Challenges', 'Meet VEX GO. An affordable construction system that teaches the fundamentals of STEM through fun, hands-on activities that help young students perceive coding and engineering in a fun and positive way!', `id`
FROM curriculum_entries, files 
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - HelloPurr: Tap the Kitty, Hear Him Meow', 'HelloPurr is a simple app that you can build in a very fun way. You will create a button that has a picture of your favorite cat on it, and then program the button so that when it is clicked a "meow" sound plays with some vibrations.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'AppInventor Mobile Apps Development Introductory'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Piccall', 'PicCall shows you how you can use App Inventor to make apps that do actual things, like calling friends. We will learn about how real-life applications work and are programmed.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'AppInventor Mobile Apps Development Introductory'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - QuizMe', 'Youll design the quiz game so that the user proceeds from question to question by clicking a Next button and receives simple correct/incorrect feedback on each answer', `id`
FROM curriculum_entries, files 
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Chameleon', 'In this project kids will make a chameleon that changes color to match its background using a webcam to take pictures of different colors, then use machine learning with those examples to train the chameleon to recognize colors.', `id`
FROM curriculum_entries, files 
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A -  Chatbots', 'In this project you will make a chatbot that can answer questions about a topic of your choice.', `id`
FROM curriculum_entries, files 
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'C -  Secret Code', 'In this project you will train the computer to understand secret code Words. You’ll use that to say commands to a spy to guide it around a town.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'A.I. & Machine Learning Intermediate'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'D- Laser Eyes', 'In this project you will make voice-powered laser eyes! This is a game where you will see laser beams shoot from your eyes in your computer’s webcam. You will use these to shoot at bottles. Your laser eyes will be voice-activated, so you will have to shout “laser eyes” to make them shoot. You will be using two kinds of machine learning model. Speech recognition to activate the lasers and face detection to aim them!', `id`
FROM curriculum_entries, files 
WHERE `description` = 'A.I. & Machine Learning Intermediate'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A -  Top Trumps', 'In this project you will train a computer to play a card game. For some values on the cards, you win by having the highest number. For others, you win by having the lowest. The range of numbers for different values will vary. The aim will be for the computer to learn how to play the game well without you having to give it a list of all the cards or tell it the rules.', `id`
FROM curriculum_entries, files 
...
-- 3D Design & Printing
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - 3D Design & Printing Introductory', 'Learn to create your very own 3D structures that can be printed in the future. At the same time, students will be able to learn about how to use TinkerCAD and its various tools, such as alignment tools and hole generation.', `id`
FROM curriculum_entries, files 
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Superhero', 'In this topic, student will create their superhero with their designed costume, wings and decoration through learning mirror tool, rotation, wokrplane. Student will also revisiting Boolean addition, duplication, scaling and grouping', `id`
FROM curriculum_entries, files 
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Micro:bit Introductory', 'The micro:bit is a small computer that is well suited for introducing how software and hardware work together to perform tasks. It has an LED light display, buttons, sensors, and many input/output features that can be coded and physically interacted with.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Micro:bit Digital Making Introductory'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Micro:bit Rock, Paper Scissors', 'Rock, paper, scissors is a simple game that everyone has played at least once in their life. But can this simple game be created using Micro:bit, the answer is yes, yes it can.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Micro:bit Digital Making Introductory'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Micro:bit Hot Potato', 'For this lesson, students will recreate the game Hot Potato using their Micro:bit. For this game, students will start a timer with a random countdown and when the timer goes off, the game is over and whoever is still holding the potato has lost.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Micro:bit Digital Making Intermediate'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Micro:bit Compass', 'A compass is an instrument with a magnetic pointer which shows the direction of the magnetic north and the bearings from it. The Micro:bit comes with a magnetometer that can be used to detect magnetic north in much the same way as a compass.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Micro:bit Digital Making Intermediate'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'C - Micro:bit Guita', 'Guitars are musical instruments that typically has six strings with history dating back to 1200s in Spain. Modern electric guitars were introduced in the 1930s and use electronic pickups and loudspeakers to amplify its sound during performances.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Micro:bit Digital Making Intermediate'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Comfort Home', 'Students consolidate their knowledge in CoSpaces through creating a special meal in special occasion of their own choice. They will be thrilled to have their own AR project and share with their family members or friends.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'AR/VR Introductory'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'C - Creative Story Remix', 'This course is suitable for kids who have experinece in CoSpaces and would like to challenge themselves. They need to rewrite novel stories with creativity and illustrate in an immersive VR environment.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'AR/VR Introductory'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'A - Interactive Art (Starry Night)', 'For these course students should have previous experience in working in CoSpaces. They are going to further explore on the potentials of VR/AR in different areas.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'AR/VR Intermediate'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Interactive Art (Pumpkin)', 'Introduce some background about Japanese Artist Yayoi Kusama. Her iconic dots in every pieces of her works. She said that " Keep creating artworks make me happy"', `id`
FROM curriculum_entries, files 
WHERE `description` = 'AR/VR Intermediate'
AND files.`physical_file_name` = '';

INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'C - Interactive Art (Self Portrait)', 'This is an extension for those who challenge themselves and have great interest in exploring the possibilities of using VR integrating into artworks. They need to review some self portraits of great artists and create a VR of their own.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'AR/VR Intermediate'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B - Jr Marine Adventure', 'What causes things to sink or float? How scientists help to explore the ocean below? In this course, students experiment with different attributes related to water, its buoyancy, pressure. How to navigate above water and help lifes below water.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Smart City Elementary'
AND files.`physical_file_name` = '';
...
INSERT INTO curriculum_course_information_entries (`icon_id`, `title`, `content`, `entry_id`)
SELECT files.`id`, 'B -  Aerospace Journey', 'Humans dream to fly in air. In this journey, students apply Engineering Design Process to design, create, test and refine a variety of flying machines. Not only to fly against gravity but also think of ways to land safely to complete their dream of space journey.', `id`
FROM curriculum_entries, files 
WHERE `description` = 'Smart City Introductory'
AND files.`physical_file_name` = '';
