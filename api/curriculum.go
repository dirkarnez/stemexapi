	...
	return func(ctx iris.Context) {
		...
		err = q.Transaction(func(tx *query.Query) error {
			...
			for i, dto := range form.Levels {
				...
				miscMaterialsNotesType, _ := tx.CurriculumCourseLessonResourceType.Where(
				...
				for j, lesson := range dto.Lessons {
					...
					for k, presentationNote := range lesson.PresentationNotes {
						...
						_, presentationNoteFileHeader, err := ctx.Request().FormFile(fmt.Sprintf("levels.%d.lessons.%d.presentation_notes.%d.presentation_note_file", i, j, k))
						if err == nil {
							file, err := utils.SaveUploadV2(presentationNoteFileHeader, &curriculumCourse.CurriculumPlanID, []string{utils.PrefixCourseResourses, curriculumEntry.Description}, s3, tx, ctx)
							if err != nil {
								return err
							}
							curriculumCourse.CurriculumPlanID = file.ID
						...
						entityPresentationNote.ResourseID = 
