			...
			for i, dto := range form.Levels {
				...
				for j, lesson := range dto.Lessons {
					...
					for k, studentNote := range lesson.StudentNotes {
						...
						_, studentNoteFileHeader, err := ctx.Request().FormFile(fmt.Sprintf("levels.%d.lessons.%d.presentation_notes.%d.file", i, j, k))
						if err == nil {
							file, err := utils.SaveUploadV2(presentationNoteFileHeader, &entityStudentNote.ResourseID, []string{utils.PrefixCourseResourses, curriculumEntry.Description}, s3, tx, ctx)
							if err != nil {
								return err
							}
							entityPresentationNote.ResourseID = file.ID
